...
def load_structured(n: int = 50) -> list[StructuredItem]:
    # Loaded as package data so it works in both editable and non-editable installs.
    text = (resources.files("inferbench") / "data" / "structured_prompts.json").read_text()
    items = [StructuredItem(**x) for x in json.loads(text)]
    return items[:n]
